...
<details>
  ...
  <ul>
    <li>Mean</li>
    Mean의 경우 E(aX) = aE(X) property를 가지고 있기 때문에 <br>
    \begin{align}
    \prod_{s=1}^{t} \sqrt{1-\beta_{t}} = \prod_{s=1}^{t} \sqrt{\alpha_{t}} = \sqrt{\bar{\alpha}_{t}} \\
    \end{align}
    <li>Variance</li>
    Variance도 마찬가지로 aVar(X) = Var(a^{2}X) property를 가지고 있지만 
    \begin{align}
    x_{t} = \sqrt{1-\beta_{t}}x_{t-1} + \beta_{t}z, z \sim \mathcal{N}(0,I)
    \end{align}
    와 같이 더해지는 형식이기 때문에 mean과 다르게 계산됩니다. t=4일 때를 예로 들어보면, <br>
    \begin{align}
