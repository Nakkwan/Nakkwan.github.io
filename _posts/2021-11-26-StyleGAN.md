...
-   Instance normalization revisited<br>
    ...
    $$
    \begin{align}
    \mathcal{w}^\prime*{ijk} = \mathcal{s}*{i}\dot\mathcal{w}^\prime*{ijk} (\dot Scaling)\\
    \sigma*{j}=\sqrt{\sum*{i,k}\mathcal{w}^\prime*{ijk}^{2}} (supp. IID)(\dot Demodulate)\\
    \mathcal{w}^\prime\prime*{ijk} = \mathcal{w}^\prime*{ijk} / \sqrt{\sum*{i,k}\mathcal{w}^\prime*{ijk}^{2} + \epsilon} (\dot Convolution weight)\\
    \end{align}
    $$
