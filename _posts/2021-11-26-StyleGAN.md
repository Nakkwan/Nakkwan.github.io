...
<li>Instance normalization revisited</li>
    ...
    \begin{align}
    \mathcal{w}^\prime_{ijk} = \mathcal{s}_{i}\dot\mathcal{w}^\prime_{ijk}\\
    \sigma_{j}=\sqrt{\sum_{i,k}\mathcal{w}^\prime_{ijk}^{2}} \\
    mathcal{w}^\prime\prime_{ijk} = \mathcal{w}^\prime_{ijk} / \sqrt{\sum_{i,k}\mathcal{w}^\prime_{ijk}^{2} + \epsilon} \\
    \end{align}
</ul>
